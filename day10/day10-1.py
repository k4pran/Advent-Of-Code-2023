...
def traverse_pipes(mat, start, distance_mat, visited=None):
    visited = set()
    queue = deque([(start, -1)])

    while queue:
        adj, depth = queue.popleft()
        if adj not in visited:
            visited.add(adj)
            distance_mat[adj[0]][adj[1]] = len(visited)

            for adj in get_adjacent_squares(mat, adj):
                queue.append((adj, depth + 1))
            # queue.extend(get_adjacent_squares(mat, adj))
